...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    */
    ...
    // let user_txs = Arc::new(Mutex::new(HashMap::<String, Sender<proto::Data>>::new()));

    // let (client_tx, mut client_rx) = tokio::sync::mpsc::channel::<proto::Packet>(1000);

    // let client_tx_clone = client_tx.clone();
    // let client_tx_clone1 = client_tx.clone();
    // let user_txs_clone = user_txs.clone();
    ...
    //         let user_txs_clone1 = user_txs_clone.clone();
    //         let client_tx_clone1 = client_tx_clone.clone();
    //         tokio::spawn(async move {
    //             let header_size = proto::Header::size().unwrap();
    //             let client_tx_clone2 = client_tx_clone1.clone();
    //             loop {
    //                 let mut readed_len = 0;
    //                 let mut buf = vec![0; header_size];
    //                 while readed_len < header_size {
    //                     let mut tmp_buf = vec![0; header_size - readed_len];
    //                     let n = match socket_reader.read(&mut tmp_buf).await {
    //                         Ok(n) if n == 0 => {
    //                             debug!("连接断开");
    //                             return;
    //                         }
    ...
    //                         info!("发送给客户:{},的数据长度:{}", data.user_id, data.data.len());
                            
    //                         {
    //                             if let Some(rx) = user_txs_clone1.lock().await.get(&data.user_id) {
    //                                 rx.send(data).await.unwrap();
    //                             }
    //                         }
    //                     }
    //                     PackType::DataEnd => {
    ...
    //             debug!("准备转发用户数据给客户端");
    //             if let Err(e) = socket_writer.write(&packet.pack().unwrap()).await {
    //                 error!("把用户数据转发给客户端出错： {:?}", e);
    //                 return;
    //             }
    //         }
    //     }
    ...
    // let user_txs_clone = user_txs.clone();
    // let client_tx_clone1 = client_tx_clone1.clone();
    // loop {
    //     let (socket, addr) = user_listener.accept().await?;
    //     let ip_port = addr.to_string();
    //     debug!("接收到请求: {}", ip_port);

    //     let (tx, mut rx) = tokio::sync::mpsc::channel::<proto::Data>(1000);

    //     {
    //         user_txs_clone
    //             .lock()
    //             .await
    //             .insert(ip_port.clone(), tx.clone());
    //     }

    //     let (mut socket_reader, mut socket_writer) = socket.into_split();
    //     // rx 有数据就发给socket_wirter
    //     tokio::spawn(async move {
    //         while let Some(data) = rx.recv().await {
    //             if let Err(e) = socket_writer.write(&data.data).await {
    //                 error!("把客户端数据发给用户出错; err = {:?}", e);
    //                 return;
    //             }
    //             debug!("成功把客户端数据发给用户");
    //         }
    ...
    //         // 在一个循环中读取数据，直到连接被关闭
    //         let n = match time::timeout(Duration::from_secs(3), socket_reader.read(&mut buf)).await
    //         {
    //             Ok(Ok(0)) => {
    //                 debug!("用户断开连接:{}", ip_port);
    //                 return;
    //             }
    //             Ok(Ok(n)) => n,
    //             Ok(Err(e)) => {
    //                 error!("读取用户数据出错:{}, {:?}", ip_port, e);
    //                 return;
    //             }
    //             Err(_) => {
    //                 // 超时处理逻辑
    //                 info!("读取用户数据超时:{}", ip_port);
    //                 return;
    //             }
    //         };
    ...
    //         let packet = proto::Packet::new(PackType::Data, Some(data)).unwrap();
    //         if let Err(e) = client_tx_clone1.send(packet).await {
    //             error!("通过 channel 发送用户数据给客户端出错:{:?}", e);
    //         }
    //     });
    // }
}
